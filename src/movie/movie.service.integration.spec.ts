...
describe('MovieService - Integration Test', () => {
  ...
  describe('findRecent', () => {
    ...
    it('should cache recent movies', async () => {
      ...
      const cacheData = await cacheManager.get('MOVIE_RECENT');

      expect(cacheData).toEqual(result);
    });
  });

  describe('findAll', () => {
    it('should return movies with correct titels', async () => {
      const dto = { title: 'Movie 12', order: ['createdAt_DESC'], take: 10 };
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException if movie does not exist', async () => {
      await expect(service.findOne(99999999)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw error if movie does not exist', async () => {
      ...
      await expect(service.update(999999, updateMovieDto)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
